...
class Solution:
    def sampleStats(self, count: list) -> list:
        ...
        if leng % 2:
            ...
            medium_two = [0,0]
            medium_pos = [leng // 2, leng//2+1]
            first_find = False
            for i in sample[::-1]:
                medium_pos[0] -= i[1]
                medium_pos[1] -= i[1]
                if medium_pos[0] <= 0 and first_find == False:
                    medium_two[0] = i[0]
                    first_find = True
                if medium_pos[1] <= 0:
                    medium_two[1] = i[0]
                    medium = (medium_two[0] + medium_two[1]) / 2
                    break
 
        return [minimum, maximum, float(mean['sum']/mean['individual']), float(medium), mode['val']]
